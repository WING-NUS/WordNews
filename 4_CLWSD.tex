...
For example, when translating the word ``little'' in ``... little kids ...'', the word alignment information in Bing Translator does not give an alignment for the word `little' but instead translates the entire multi-word unit ``little kids''. 
...
As such, the translation would not match any of the annotations provided by our annotators. This is an appropriate treatment since a user of an educational app requesting a translation for the single word ``little'' should not see the translation of the entire phrase.
