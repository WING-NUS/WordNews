...
individual words only have few training examples. We note other attempts to use neural networks for WSD may have run into the same problem. Taghipour \& Ng \shortcite{Taghipour15} indicated the need to prevent overfitting while using a neural network to adapt C\&W embeddings by omitting a hidden layer and adding a Dropout layer, while K{\aa}geb{\"a}ck \& Salomonsson \shortcite{kaageback2016word} developed a new regularization technique in their work.
