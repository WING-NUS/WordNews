...
{\bf Knowlege level 2}: This indicates that the user has known this word for 3 times. Therefore, the testing is expected to be harder. Our system first randomly selects two words from the same news category. For the third distractor, the system keeps randomly selecting distractor from the same category, computing its semantic distance to the target word, and stops until meeting a difficulty one.


{\bf Knowledge level 3}: This indicates that the user already has a good understanding of the word, {\it i.e.}, passing the test for 6 times. As such, we make the test even harder, and choose  three difficulty distractors from the new category. 
...
To compare with our proposed method, we further implemented an existing distractor generation method  used in WordGap system~\cite{Knoop2013}. WordGap still adopts knowledge-based approach, selecting the synonyms of synonyms (computed in WordNet) as the distractors. That is, we select the most frequently used word, let's call it w1, from the target word’s synonym set. Then we select the synset, let's call it s1, which the synset where the most frequently used word of w1 lies in. Synset s1 will now contain all the words that are synonyms of synonyms of the target word, we will select the 3 most frequently used words are the three distractors in baseline approach.
